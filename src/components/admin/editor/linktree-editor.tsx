...
function parseGradient(bgValue: string) {
  ...
  const match = bgValue.match(
    /linear-gradient\((\d+)deg,\s*(#[a-fA-F0-9]+|rgba?\(.*?\)),\s*(#[a-fA-F0-9]+|rgba?\(.*?\))\)/i,
  );
  if (match) {
        ...
        <div className="flex gap-4 pt-4 border-t mt-2">
          <div className="flex items-center gap-2 flex-1">
            ...
            <div className="flex w-full gap-2">
              <Input
                type="color"
                value={link.bg_color.length === 7 ? link.bg_color : "#ffffff"}
                onChange={(e) =>
                  updateLink(link.id, "bg_color", e.target.value)
                }
                className="w-8 h-8 p-0 border-0 bg-transparent rounded-md cursor-pointer shrink-0"
              />
              <Input
                value={link.bg_color}
                onChange={(e) =>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Input
              ...
              onChange={(e) =>
                updateLink(link.id, "text_color", e.target.value)
              }
              className="w-8 h-8 p-0 border-0 bg-transparent rounded-md cursor-pointer"
            />
          </div>
        </div>
      </div>
...
export default function LinktreeEditor({
  ...
  const [data, setData] = useState<LinktreeData>({
    ...
    bg_value:
      initialData?.bg_type === "gradient" &&
      !initialData?.bg_value?.startsWith("linear-gradient")
  ...
  const updateLink = useCallback(
    (id: string, field: keyof Link, value: any) => {
      setData((prev) => ({
        ...prev,
  ...
  const handleGradientChange = (angle: number, c1: string, c2: string) => {
    ...
    setData((prev) => ({
      ...prev,
      bg_value: `linear-gradient(${angle}deg, ${c1}, ${c2})`,
    }));
  };
  ...
  const handleFileUpload = async (
    ...
    if (error || !url) {
      alert(
        "שגיאה בהעלאת התמונה. וודא שסקריפט מסד הנתונים הורץ. שגיאה: " +
              ...
              >
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div
                      className="flex items-stretch bg-background border rounded-md overflow-hidden"
                      dir="ltr"
                    >
                      <span className="text-muted-foreground px-3 flex items-center bg-muted border-r text-sm">
                        site.com/
                      </span>
                      <Input
                        id="slug"
                  ...
                  <div className="space-y-4 pt-4 border-t">
                    ...
                    <div className="flex items-center justify-between bg-muted/50 p-3 rounded-lg border">
                      ...
                      >
                        הסתר לוגו / אות ראשונה
                      </Label>
                      <Switch
                        id="hide-logo"
                        checked={data.logo_url === "hidden"}
                        onCheckedChange={(c) =>
                          handleChange("logo_url", c ? "hidden" : "")
                        }
                      />
                    </div>
                      ...
                      <div className="flex gap-4 items-center mt-4">
                        ...
                        <div className="flex-1 space-y-2">
                          <Input
                            ...
                            placeholder="כתובת תמונה ברשת (URL)"
                            dir="ltr"
                          ...
                          <div className="relative">
                            ...
                            >
                              {isUploadingLogo ? (
                                ...
                                <>
                                  <Upload className="w-4 h-4 mr-2" /> העלאת
                                  תמונה מהמחשב
                                </>
                              )}
                            </Button>
                ...
                <div className="space-y-4 mt-6">
                  ...
                  {data.links.length === 0 && (
                    <p className="text-center text-neutral-500 text-sm">
                      אין קישורים. הוסף קישור או השתמש בתבניות למעלה.
                    </p>
                  )}
                </div>
              </TabsContent>
                    ...
                    <div className="space-y-4 p-4 bg-neutral-900/50 rounded-lg border border-neutral-800">
                      ...
                      <div className="flex items-center gap-3">
                        <Input
                          type="color"
                          value={
                            data.bg_value.length === 7
                              ? data.bg_value
                              : "#0e0e0e"
                          }
                          onChange={(e) =>
                            handleChange("bg_value", e.target.value)
                          }
                          className="w-10 h-10 p-0 border-0 bg-transparent rounded-md cursor-pointer"
                        />
                        <span
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            className="w-full h-10 p-0 border-0 bg-transparent rounded-md cursor-pointer"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-xs text-neutral-400">
                    ...
                    <div className="space-y-2 p-4 bg-neutral-900/50 rounded-lg border border-neutral-800">
                      ...
                      <div className="space-y-2">
                        <Input
                          ...
                          placeholder="כתובת תמונה ברשת (URL)"
                          dir="ltr"
                          className="bg-neutral-950 border-neutral-800 text-left text-sm"
                        />
                        <div className="relative mt-2">
                          <Input
                            type="file"
                            accept="image/*"
                            onChange={(e) => handleFileUpload(e, "bg_value")}
                            className="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10"
                          />
                          <Button
                            variant="secondary"
